...
const clientsList = [
  ...
  { name: 'Pilot Client', description: 'Plugin for pilots connecting to VATSIM.', available: false },
  { name: 'Account', description: 'Manage your account settings and preferences.', available: false },
  { name: 'Settings', description: 'Configure your application settings.', available: false },
];
      ...
      <div className="relative min-h-screen w-full flex overflow-hidden">
        ...
        >
          <div className="flex flex-col space-y-2">
            <h2 className="text-white text-xl font-bold mb-2 text-center">{accountView ? 'Account' : 'Applications'}</h2>
            <AnimatePresence mode="wait">
              {!accountView ? (
                clientsList.slice(0, 4).map((client, index) => (
                  <motion.button
                    key={index}
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                ...
                <>
                  ...
                  {clientsList.slice(4).map((client, index) => (
                    <motion.button
                      key={index}
                      initial={{ opacity: 0 }}
                      animate={{ opacity: 1 }}
        ...
        <motion.main className="flex-1 min-h-screen p-6 sm:p-8 flex flex-col items-center justify-center text-slate-300 relative overflow-hidden">
          ...
          {(selectedClient.name !== 'vatSys Plugin') && (
            <div
              ...
              style={{
                backgroundImage: selectedClient.name === 'Home'
          ...
          <AnimatePresence mode="wait">
            ...
            >
              {selectedClient.name === 'Home' ? (
                ...
                <>
                  <h1 className={`text-3xl sm:text-4xl font-bold mb-4 ${selectedClient.name === 'vatSys Plugin' ? 'text-blue-400' : selectedClient.name === 'Euroscope Plugin' ? 'text-emerald-400' : selectedClient.name === 'Pilot Client' ? 'text-violet-400' : 'text-white'}`}>{selectedClient.name}</h1>
                  <p className="text-slate-400 mb-2 mt-2 max-w-xl text-center px-4">{selectedClient.description}</p>
                  {(selectedClient.name === 'vatSys Plugin' || selectedClient.name === 'Euroscope Plugin' || selectedClient.name === 'Pilot Client') && (
